...
public class ThumbnailCommand extends Command {
	...
	public void initParams(final HttpServletRequest request,
		...
		try {
			this.fileName =new String( request.getParameter("FileName").getBytes("ISO-8859-1"),"UTF-8");
		
		
			this.ifModifiedSince = Long.valueOf(request
	...
	private void createThumb() throws ConnectorException {
		...
		try {
			if (!thumbFile.exists()) {
				File orginFile = new File(configuration.getTypes()
						.get(this.type).getPath()
